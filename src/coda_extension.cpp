...
CreateCodaSecretFromConfig(ClientContext &, CreateSecretInput &input) {
  ...
  for (auto &named_param : input.options) {
    ...
    if (lower_name == "token") {
      ...
      throw InvalidInputException("Unknown named parameter for Coda secret: %s",
                                  lower_name);
    }
  }
...
static void RegisterCodaSecretType(ExtensionLoader &loader) {
  ...
  secret_type.name = Identifier(CODA_SECRET_TYPE);
  secret_type.deserializer = KeyValueSecret::Deserialize<KeyValueSecret>;
  secret_type.default_provider = "config";
  secret_type.extension = "coda";
  loader.RegisterSecretType(secret_type);

  CreateSecretFunction config_fun = {CODA_SECRET_TYPE, "config",
                                     CreateCodaSecretFromConfig};
...
static void LoadInternal(ExtensionLoader &loader) {
  ...
  StorageExtension::Register(loader.GetDatabaseInstance().config, "coda",
                             storage);
}

void CodaExtension::Load(ExtensionLoader &loader) { LoadInternal(loader); }

std::string CodaExtension::Name() { return "coda"; }

} // namespace duckdb

extern "C" {

DUCKDB_CPP_EXTENSION_ENTRY(coda, loader) { duckdb::LoadInternal(loader); }
}
