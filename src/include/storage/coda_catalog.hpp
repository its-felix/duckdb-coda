...
public:
	CodaCatalog(AttachedDatabase &db, ClientContext &context, string doc_id, string token, string api_base,
	...
	const vector<CodaTableInfo> &Tables() const {
		return tables;
	}
	...
	const string &APIBase() const {
		return api_base;
	}
	bool IncludeRowMetadata() const {
		return include_row_metadata;
	}
