...
struct CodaScanBindData : FunctionData {
	CodaScanBindData(TableCatalogEntry &table_entry_p, string doc_id_p, string token_p, string api_base_p,
	                 CodaTableInfo table_p)
	    : table_entry(table_entry_p), doc_id(std::move(doc_id_p)), token(std::move(token_p)),
	      api_base(std::move(api_base_p)), table(std::move(table_p)) {
	}

	unique_ptr<FunctionData> Copy() const override {
		auto copy = make_uniq<CodaScanBindData>(table_entry, doc_id, token, api_base, table);
	...
	bool Equals(const FunctionData &other_p) const override {
		...
		return &table_entry == &other.table_entry && doc_id == other.doc_id && api_base == other.api_base &&
