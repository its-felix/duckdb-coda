...
CodaCatalog::CodaCatalog(AttachedDatabase &db, ClientContext &context, string doc_id_p, string token_p,
                         string api_base_p, bool include_row_metadata_p)
    : Catalog(db), doc_id(std::move(doc_id_p)), token(std::move(token_p)), api_base(std::move(api_base_p)),
...
void CodaCatalog::LoadCatalog(ClientContext &context) {
	...
	main_schema = make_uniq<CodaSchemaCatalogEntry>(context, *this, schema_info, tables);
}
...
optional_ptr<SchemaCatalogEntry> CodaCatalog::LookupSchema(CatalogTransaction, const EntryLookupInfo &schema_lookup,
...
void CodaCatalog::ScanSchemas(ClientContext &, std::function<void(SchemaCatalogEntry &)> callback) {
	callback(*main_schema);
}

optional_ptr<CatalogEntry> CodaCatalog::CreateSchema(CatalogTransaction, CreateSchemaInfo &) {
...
PhysicalOperator &CodaCatalog::PlanInsert(ClientContext &, PhysicalPlanGenerator &planner, LogicalInsert &op,
	...
	auto &insert = planner.Make<CodaInsert>(op, op.table.Cast<CodaTableCatalogEntry>());
	insert.children.push_back(*plan);
	return insert;
}

PhysicalOperator &CodaCatalog::PlanCreateTableAs(ClientContext &, PhysicalPlanGenerator &, LogicalCreateTable &,
                                                 PhysicalOperator &) {
	throw NotImplementedException("Coda DDL is not supported: CREATE TABLE AS");
}

PhysicalOperator &CodaCatalog::PlanUpdate(ClientContext &, PhysicalPlanGenerator &planner, LogicalUpdate &op,
                                          PhysicalOperator &plan) {
	if (op.return_chunk) {
		throw NotImplementedException("RETURNING is not supported for Coda UPDATE");
	}
	auto &update = planner.Make<CodaUpdate>(op, op.table.Cast<CodaTableCatalogEntry>());
	update.children.push_back(plan);
	return update;
}

PhysicalOperator &CodaCatalog::PlanDelete(ClientContext &, PhysicalPlanGenerator &planner, LogicalDelete &op,
                                          PhysicalOperator &plan) {
	if (op.return_chunk) {
		throw NotImplementedException("RETURNING is not supported for Coda DELETE");
	}
	auto &bound_ref = op.expressions[0]->Cast<BoundReferenceExpression>();
	auto &del = planner.Make<CodaDelete>(op, op.table.Cast<CodaTableCatalogEntry>(), bound_ref.index);
	del.children.push_back(plan);
	return del;
}

unique_ptr<LogicalOperator> CodaCatalog::BindCreateIndex(Binder &, CreateStatement &, TableCatalogEntry &,
...
bool CodaCatalog::InMemory() {
	return false;
}

string CodaCatalog::GetDBPath() {
	return doc_id;
}
