...
public:
	CodaScanGlobalState(ClientContext &context, const CodaScanBindData &bind_data, TableFunctionInitInput &input)
	    : client(context, bind_data.doc_id, bind_data.token, bind_data.api_base), table(bind_data.table) {
...
static unique_ptr<GlobalTableFunctionState> CodaScanInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
...
static string CodaQueryLiteral(const Value &value) {
	...
	default: {
		...
		if (!doc) {
			throw InternalException("Failed to construct Coda query literal");
		}
		auto root = yyjson_mut_strncpy(doc, string_value.c_str(), string_value.size());
		yyjson_mut_doc_set_root(doc, root);
		size_t len;
		auto data = yyjson_mut_val_write_opts(root, YYJSON_WRITE_ALLOW_INF_AND_NAN, nullptr, &len, nullptr);
		if (!data) {
			yyjson_mut_doc_free(doc);
			throw InternalException("Failed to serialize Coda query literal");
		}
...
static bool TryExtractCodaEqualityFilter(const LogicalGet &get, const CodaTableInfo &table, const Expression &expr,
...
static void CodaScanPushdownComplexFilter(ClientContext &, LogicalGet &get, FunctionData *bind_data_p,
	...
	for (idx_t filter_idx = 0; filter_idx < filters.size(); filter_idx++) {
		string query;
		string description;
		if (!TryExtractCodaEqualityFilter(get, bind_data.table, *filters[filter_idx], query, description)) {
			continue;
		}
...
static Value CellToValue(const CodaCellValue &cell, const LogicalType &target_type) {
	if (cell.type == CodaJSONValueType::INVALID || cell.type == CodaJSONValueType::JSON_NULL) {
	...
	try {
		...
		case LogicalTypeId::BOOLEAN:
			if (cell.type == CodaJSONValueType::BOOLEAN) {
...
static Value MetadataToValue(const string &value, const LogicalType &target_type) {
	if (value.empty()) {
		return Value(target_type);
	}
	try {
...
static void CodaScan(ClientContext &, TableFunctionInput &input, DataChunk &output) {
	...
	while (out_row < STANDARD_VECTOR_SIZE) {
		if (state.row_offset >= state.rows.size()) {
			...
			if (state.next_page_token.empty() && !state.next_sync_token.empty() && !state.sync_check_done) {
		...
		for (idx_t out_col = 0; out_col < state.column_indexes.size(); out_col++) {
			...
			if (column.row_metadata) {
				if (StringUtil::CIEquals(column.id, "createdAt")) {
					output.data[out_col].SetValue(out_row, MetadataToValue(row.created_at, column.duckdb_type));
				} else if (StringUtil::CIEquals(column.id, "updatedAt")) {
					output.data[out_col].SetValue(out_row, MetadataToValue(row.updated_at, column.duckdb_type));
				} else {
			...
			if (entry == row.values.end()) {
				...
				output.data[out_col].SetValue(out_row, CellToValue(entry->second, column.duckdb_type));
			}
		}
		out_row++;
		...
		if (state.row_offset >= state.rows.size() && state.next_page_token.empty()) {
...
static virtual_column_map_t CodaScanGetVirtualColumns(ClientContext &, optional_ptr<FunctionData>) {
	virtual_column_map_t result;
	result.insert(make_pair(COLUMN_IDENTIFIER_ROW_ID, TableColumn("rowid", LogicalType::VARCHAR)));
	return result;
}

static vector<column_t> CodaScanGetRowIdColumns(ClientContext &, optional_ptr<FunctionData>) {
	return {COLUMN_IDENTIFIER_ROW_ID};
}

static BindInfo CodaScanGetBindInfo(const optional_ptr<FunctionData> bind_data) {
...
static InsertionOrderPreservingMap<string> CodaScanToString(TableFunctionToStringInput &input) {
	auto &bind_data = input.bind_data->Cast<CodaScanBindData>();
...
TableFunction CodaScanFunction::GetFunction() {
	TableFunction function("coda_scan", {}, CodaScan, nullptr, CodaScanInitGlobal);
