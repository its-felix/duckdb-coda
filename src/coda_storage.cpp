...
static string GetAttachOption(AttachOptions &attach_options, const string &name) {
...
static bool GetBooleanAttachOption(AttachOptions &attach_options, const string &name) {
...
static string ResolveToken(ClientContext &context, const string &doc_id, AttachOptions &attach_options) {
	...
	auto match = secret_manager.LookupSecret(transaction, "coda:" + doc_id, "coda");
	if (!match.HasMatch()) {
	...
	throw InvalidInputException("Coda token not provided. Pass TOKEN in ATTACH or create a Coda secret.");
}

static unique_ptr<Catalog> CodaAttach(optional_ptr<StorageExtensionInfo>, ClientContext &context, AttachedDatabase &db,
                                      const string &, AttachInfo &info, AttachOptions &attach_options) {
	...
	auto include_row_metadata = GetBooleanAttachOption(attach_options, "include_row_metadata");
	return make_uniq<CodaCatalog>(db, context, doc_id, token, api_base, include_row_metadata);
}

static unique_ptr<TransactionManager> CodaCreateTransactionManager(optional_ptr<StorageExtensionInfo>,
